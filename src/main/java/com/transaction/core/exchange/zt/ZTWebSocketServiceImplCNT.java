...
@Slf4j
@Service("ztWebSocketServiceCNT")
@ConditionalOnExpression("${zt.enabled:true}")
public class ZTWebSocketServiceImplCNT implements WebSocketService {

    private final String url = "wss://ws.zt.com/ws";

    @Resource(name = "ztWebSocketClientCNT")
    @Lazy
    ...
    public void init(Map<String,List<String>> symbolMap) {
        ...
        for(Map.Entry<String,List<String>> entry : symbolMap.entrySet()){
            for(String s : entry.getValue()){
                ...
                symbols.add(s + "_" + "CNT");
                symbols.add(entry.getKey() + "_" + "CNT");
            }
        }
        // 启动主题订阅线程
